...
# skip if locked,
print <<EOF
jump HANDLESWITCH notEqual currentType \@memory-cell
read locked currentBlock ${\($fieldno{"locked"})}
jump NEXT greaterThan locked \@tick
...
# read if I'm older
print "jump NEXT always x false\n";
print "READCELL: ";
for (@fields) {
...
# if not an unloader,
print <<EOF
HANDLESWITCH: jump MAYBECORE notEqual currentType \@switch
sensor disable currentBlock \@enabled
jump NEXT always x false
EOF
;
print <<EOF
...
print "op sub itemCapacity itemCapacity 500\n";
print "op mul disableAdd disable 1000\n";
print "op add itemCapacity itemCapacity disableAdd\n";
